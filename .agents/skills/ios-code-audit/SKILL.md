...
- **`Dead/` (or any explicitly-archived directory) is excluded.** Check `AGENTS.md` / project README for any "do not edit" directories before launching agents.
...
Read `AGENTS.md` if present — it usually flags central state files (e.g., `AppState.swift`, `CaptureState.swift`), the rendering pipeline, and any intentionally-excluded directories.
...
- **Agent B — Dead code, duplication, refactor candidates.** Feed it the hot-spot list and any known-stale files from `AGENTS.md`.
